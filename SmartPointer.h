...
public:
  ...
  AutoPointer(value &&src) { MoveImpl(this, &src); }

  ~AutoPointer() { DeleteImpl(this); }
...
private:
  void CopyImpl(value *lhs, value *rhs) { lhs->reset(new T(*rhs->Data)); }
  ...
  void DeleteImpl(value *src) { src->reset(nullptr); }
...
public:
  ...
  SharedPointer(T *ptr) : Data(ptr), previous(nullptr), next(nullptr) {}

  SharedPointer(const value &src) : SharedPointer(nullptr) {
    CopyImpl(this, const_cast<value *>(&src));
  }

  SharedPointer(value &&src) { MoveImpl(this, &src); }

  ~SharedPointer() { DeleteImpl(this); }
...
public:
  ...
  UniquePointer(value &&src) { MoveImpl(this, &src); }

  ~UniquePointer() { DeleteImpl(this); }
...
template <typename T> using Auto = AutoPointer<T>;

template <typename T, typename... Ts> Auto<T> MakeAuto(Ts... args) {
  return std::move(MakeImpl<Auto, T, Ts...>(args...));
}

template <typename T> using Shared = SharedPointer<T>;

template <typename T, typename... Ts> Shared<T> MakeShared(Ts... args) {
  return std::move(MakeImpl<Shared, T, Ts...>(args...));
}

template <typename T> using Unique = UniquePointer<T>;

template <typename T, typename... Ts> Unique<T> MakeUnique(Ts... args) {
